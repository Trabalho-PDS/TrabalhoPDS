...
std::vector<std::string> RepositorioUsuarios::splitString(const std::string &input, char delimiter) {
...
void RepositorioUsuarios::criar_usuario(const std::string email,
  ...
  std::ofstream FileUsers =
      openFileUsers<std::ofstream>(RepositorioUsuarios::_filePath, std::ios::app);
...
bool RepositorioUsuarios::autenticar_login(std::string email,
  ...
  std::ifstream FileUsers(_filePath);

  if (!FileUsers.is_open()) {
    std::cout << "Erro ao abrir o arquivo para ler: " << _filePath << '\n';
    return false;
  }
